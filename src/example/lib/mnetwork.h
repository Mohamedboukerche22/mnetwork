                    ...
                    if (it != routes_.end()) {
                        ...
                        for (const auto& [pattern, handler] : routes_) {
                            if (pattern.find('*') != string::npos ||
                                ...
                                // Simple pattern matching (you can implement full regex)
...
server.route("/api/data", [](const mnetwork::HttpRequest& req, mnetwork::HttpResponse& res) {
    ...
    res.body = R"({"message": "Hello from API"})";
});
server.run();
...
// Example 5: Simple hosting (backward compatible)
mnetwork::host("<h1>Hello World!</h1>", 3000);
